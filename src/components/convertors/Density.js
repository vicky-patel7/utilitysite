...
import { convertDensity, densityUnitOptions } from '../../../src/utils/densityUtils';

const unitOptions = Object.keys(densityUnitOptions);

const Density = () => {

  const [sourceUnit, setSourceUnit] = useState('kgPerMetercube');
  const [targetUnit, setTargetUnit] = useState('kgPerMetercube');
  const [sourceValue, setSourceValue] = useState();
  const [result, setResult] = useState(0);

  useEffect(() => {
    try {
      setResult(convertDensity(sourceUnit, targetUnit, sourceValue));
    } catch (e) {
